...
class Stage {
    ...
    doAttack(attacking, attacked) {
        if (attacking.life <= 0 || attacked.life <= 0) {
            this.log.addMessage("Já há algum morto");
        ...
        if (actualAttack > actualDefense) {
            ...
            this.log.addMessage(`${attacking.name} deu ${actualAttack.toFixed(2)} de dano em ${attacked.name}`);
        } else {
...
class Log {
    ...
    render() {
        ...
        if (this.list.length >= 1) {
            this.listEL.innerHTML += `<li>${this.list[0]}</li>`
        }
    }
